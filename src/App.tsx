  ...
  const handleStartBattle = async (username1: string, username2: string) => {
    ...
    try {
      const [user1, user2] = await Promise.all([
        generateDummyUserData(username1),
        generateDummyUserData(username2)
      ])
      setPlayer1(user1)
      setPlayer2(user2)
      setShowBattle(true)
...
function SelectionScreen({ onStartBattle, isLoading }: { onStartBattle: (u1: string, u2: string) => Promise<void>, isLoading: boolean }) {
